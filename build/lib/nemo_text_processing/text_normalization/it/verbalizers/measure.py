...
from nemo_text_processing.text_normalization.it.graph_utils import ones
...
class MeasureFst(GraphFst):
    ...
    def __init__(self, decimal: GraphFst, cardinal: GraphFst, fraction: GraphFst, deterministic: bool):
        ...
        unit_masc = (unit_plural_masc | unit_singular_masc) + pynini.closure(
            NEMO_WHITE_SPACE + "per" + pynini.closure(NEMO_NOT_QUOTE, 1), 0, 1
        )
        unit_masc |= "per" + pynini.closure(NEMO_NOT_QUOTE, 1)
        ...
        graph_masc |= graph_fraction_masc + NEMO_WHITE_SPACE + pynutil.insert("di ") + unit_masc
        graph_masc |= pynutil.add_weight(
            graph_fraction_masc @ (NEMO_SIGMA + pynini.union("mezzo", "mezzi")) + NEMO_WHITE_SPACE + unit_masc, -0.001
        )  # "medio litro" not "medio de litro"

        graph_fem = (graph_cardinal_fem | graph_decimal_fem) + NEMO_WHITE_SPACE + unit_fem
        graph_fem |= graph_fraction_fem + NEMO_WHITE_SPACE + pynutil.insert("di ") + unit_fem
        graph_fem |= pynutil.add_weight(
            graph_fraction_fem @ (NEMO_SIGMA + pynini.union("mezza", "mezze")) + NEMO_WHITE_SPACE + unit_fem, -0.001
        )
        ...
        graph = (
            pynini.cdrewrite(
                pynutil.insert(" si"), "quantity: \"" + pynini.closure(NEMO_NOT_QUOTE, 1), "\"", NEMO_SIGMA
            )
            @ graph
        )  # billones de xyz

        graph @= pynini.cdrewrite(pynini.cross(ones, "uno"), "", NEMO_WHITE_SPACE + "per", NEMO_SIGMA)
