...
class TimeFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        morning_phrases = pynini.cross("am", "di mattina")
        afternoon_phrases = pynini.cross("pm", "di sera")
        evening_phrases = pynini.cross("pm", "di notte")

        # For the 12's
        mid_times = pynini.accep("dodici")
        mid_phrases = (
            pynini.string_map([("pm", "di mezzogiorno"), ("am", "di notte")])
            if deterministic
        ...
        if not deterministic:
            ...
            graph_para = minute + pynutil.insert(" alle ") + delete_space + hour + graph_style_2
            graph_para |= (
                minute
                + pynutil.insert(" alle ")
                + delete_space
